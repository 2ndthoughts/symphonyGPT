...
class SentimentAllNegative(OutcomeStrategy):
    ...
    def get_outcome_prompt(self):
        return "Parse the following array of JSON objects and select the one where the 'label' attribute value is 'NEGATIVE'"

    def process_outcome(self, conductor, prompt, response_array=None):
        return super().process_stat_outcome('filter', 'label', response_array, 'NEGATIVE')